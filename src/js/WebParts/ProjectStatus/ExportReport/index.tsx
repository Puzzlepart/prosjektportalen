...
export default class ExportReport extends React.Component<IExportReportProps, IExportReportState> {
    ...
    private renderExportBtn = exportStatus => {
        ...
        return (
            <PrimaryButton
                ...
                {exportStatus === ExportReportStatus.hasExported ? Localization.getResource("ProjectStatus_SnapshotIsSaved") : Localization.getResource("ProjectStatus_SaveSnapshot")}
            </PrimaryButton>
        );
    }
    ...
    private saveFileToLibrary = (libraryRelativeUrl: string, fileName: string, title: string, fileBlob: Blob): Promise<any> => {
        return pnp.sp.web.getFolderByServerRelativeUrl(libraryRelativeUrl).files.add(fileName, fileBlob, true).then((fileAddResult) => {
            return fileAddResult.file.listItemAllFields.get().then((fileAllFields) => {
                return pnp.sp.web.lists.getByTitle(Localization.getResource("Lists_ProjectStatus_Title")).items.getById(fileAllFields.Id).update({
                    "Title": title,
                });
            });
        });
    ...
    private fetchReports = () => new Promise<any[]>((resolve, reject) => {
        pnp.sp.web.lists.getByTitle(Localization.getResource("Lists_ProjectStatus_Title"))
            .items
    ...
    private getReportOptions = (reports): Array<IDropdownOption> => {
        ...
        let firstOption = reports.length > 0
            ? {
                key: "01",
                text: String.format(Localization.getResource("ProjectStatus_SnapshotHistory"), reports.length),
            }
            : {
                key: "01",
                text: Localization.getResource("ProjectStatus_SnapshotNoHistory"),
            };
        return [
            firstOption,
            ...options,
    ...
    private saveReport = reportBlob => {
        ...
        this.saveFileToLibrary(`${_spPageContextInfo.webServerRelativeUrl}/${Localization.getResource("Lists_ProjectStatus_Title")}`, fileName, fileTitle, reportBlob).then((data) => {
            this.setState({
