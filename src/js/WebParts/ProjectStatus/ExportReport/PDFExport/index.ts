...
export default class PDFExport {
    ...
    public constructor(sections: SectionModel[], layout = 'l', font = 'Helvetica') {
    ...
    public async addPageWithImage(imageId: string, pageTitle: string) {
        const html2canvasR = html2canvas as any as (element: HTMLElement, options?: Partial<html2canvas.Options>) => Promise<HTMLCanvasElement>
    ...
    */
    public async addProjectPropertiesSection(section: SectionModel): Promise<void> {
        ...
        section.viewFields
            ...
            .forEach((viewField, index) => {
                ...
                const field = project.fields.filter((_) => _.InternalName === viewField)[0]
                const value = project.item[field.InternalName]
    ...
    private addDocumentTitle = (value: string, yPosition: number, xPosition: number) => {
        ...
        const splitValue = this.jspdf.splitTextToSize(value, 270)
    ...
    private fetchDataForSection = (section: SectionModel) => new Promise<any>((resolve, reject) => {
        ...
        ctx.executeQueryAsync(() => {
            const validViewFields = section.viewFields.filter(vf => _fields.get_data().filter(lf => lf.get_internalName() === vf).length > 0)
            const columns: Array<ITableColumn> = validViewFields.map(vf => {
            ...
            const items = _items.get_data().map(i => i.get_fieldValuesAsText().get_fieldValues())
    ...
    private fetchProjectData = (project: IProject, configListTitle = __.getResource('Lists_ProjectConfig_Title')) => new Promise<Partial<any>>((resolve, reject) => {
        ...
        Promise.all([configPromise])
            .then(([config]) => {
                const itemFieldNames = Object.keys(project.item)
