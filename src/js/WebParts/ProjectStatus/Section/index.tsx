...
export default class Section extends React.PureComponent<ISectionProps, ISectionState> {
                    ...
                    <div
                        ...
                        <div className='field-section-row'>
                            {section.viewFields.map((vf, key) => {
                                const [field] = fields.filter(f => f.InternalName === vf)
                                if (!field) {
                                    return null
                                }
                                return (
    ...
    private renderCustomComponent(customComponentName: string): JSX.Element {
        const customComponent = GetWebPartComponentByName(customComponentName)
        if (customComponent) {
    ...
    private fetchListData({ section }: ISectionProps) {
        return new Promise<ISectionListData>((resolve, reject) => {
            ...
            ctx.executeQueryAsync(() => {
                const items = _items.get_data().map(i => i.get_fieldValuesAsHtml().get_fieldValues())
                const validViewFields = section.viewFields.filter(vf => _fields.get_data().filter(lf => lf.get_internalName() === vf).length > 0)
                const columns = validViewFields.map(vf => {
