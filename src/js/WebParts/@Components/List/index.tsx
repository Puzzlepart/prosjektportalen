...
import { makeUrlRelativeToSite } from "../../../Util"
        ...
        return (
            <div>
                ...
                <div hidden={!this.props.showSearchBox}>
                    <SearchBox placeholder={__.getResource("ExperienceLog_SearchBox_Placeholder")} onChanged={this._onSearch} />
                </div>
                <DetailsList
                    items={items}
                    columns={columns}
    ...
    private _onRenderItemColumn(item: any, index: number, column: IColumn) {
        ...
        switch (column.key) {
            case "Title": {
                ...
                if (path) {
                    return (
                        <ModalLink label={colValue} url={path} options={{ HideRibbon: true }} />
                    )
                } else {
                    return colValue
                }
