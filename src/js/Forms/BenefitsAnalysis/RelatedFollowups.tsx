...
const FollowupElement = ({ data: { ID, GtMeasurementDate: Date, GtMeasurementComment: Comment, GtMeasurementValue: Value } }) => {
    let dispFormUrl = `../../${__("DefaultView_BenefitsFollowup_Url")}?ID=${ID}`.replace("AllItems", "DispForm");
    return (
    <li>
        <h3>
            <ModalLink 
            label={Util.dateFormat(Date, "LL")} 
            url={dispFormUrl}
        ...
        hidden={!Comment}><b>{__("String_Comment")}:</b> {Comment}</p>
    </li>
    );
};
