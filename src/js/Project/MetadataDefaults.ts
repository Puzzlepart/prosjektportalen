        ...
        case 'TaxonomyMulti': {
            ...
            if (value.getEnumerator) {
                ...
                while (termEnumerator.moveNext()) {
                    const currentTerm = Util.getSafeTerm(termEnumerator.get_current())
            ...
            if (value._Child_Items_) {
                value._Child_Items_.forEach(term => {
                    const safeTerm = Util.getSafeTerm(term)
...
export async function SetMetadataDefaultsForLibrary(fields: IIMetadataDefaultsField[], libTitle = CONFIGURATION.DOCUMENT_LIBRARY): Promise<void> {
    ...
    const defaultValues: IMetadataDefaultsDefaultValue[] = fields
        ...
        .map(({ fieldName, fieldType }) => {
            const fieldValue = getFieldValueForType(wpFieldValues[fieldName], fieldType)
            return { fieldName, fieldValue }
        })

    const metadataDefaults = GenerateMetadataDefaults(folderServerRelativeUrl, defaultValues)
...
export async function EnsureLocationBasedMetadataDefaultsReceiverForLibrary(type = 'ItemAdded', libTitle = CONFIGURATION.DOCUMENT_LIBRARY): Promise<void> {
    ...
    const eventReceiverExists = eventReceivers.get_data().filter(er => er.get_receiverName() === recName).length > 0
    if (!eventReceiverExists) {
        const eventRecCreationInfo = new SP.EventReceiverDefinitionCreationInformation()
